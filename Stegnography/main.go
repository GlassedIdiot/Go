...
func init() {
	...
	flags.BoolVar(
		...
		"inject",
		false,
		"Enable this to inject data at the offset location specified",
	)
	flags.StringVar(
		&opts.Payload,
		"payload",
		"",
		"Payload is data that will be read as a byte stream",
	)
...
func usage() {
	fmt.Fprintf(
		os.Stderr,
		"Example Usage: %s -i in.png -o out.png --inject --offset 0x85258 --payload 1234\n",
		os.Args[0],
	)
	fmt.Fprintf(
		os.Stderr,
		"Example Encode Usage: %s -i in.png -o encode.png --inject --offset 0x85258 --payload 1234 --encode --key secret\n",
		os.Args[0],
	)
	fmt.Fprintf(
		os.Stderr,
		"Example Decode Usage: %s -i encode.png -o decode.png --offset 0x85258 --decode --key secret\n",
		os.Args[0],
	)
